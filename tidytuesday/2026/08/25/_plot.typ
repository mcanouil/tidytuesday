...
// gives the same order. Each credit row holds the chair and the person in it; a
// name credited twice on one song appears once, so a row means "this person is
// on this hit".
#let credits = {
  let rows = ()
  for role in roles {
    for song in songs {
      for person in names-in(song, role.key).dedup() {
        rows.push((role: role.name, person: person))
      }
    }
  }
  count(rows, "role", "person").sorted(key: c => (-c.n, c.person))
}
...
#let covered = roles.map(r => {
  let order = credits-for(r.name).map(c => c.person)
  let rank-of = (:)
  ...
  let uncredited = 0
  for row in songs {
    let ranks = names-in(row, r.key).map(n => rank-of.at(n))
    if ranks.len() == 0 {
      uncredited += 1
      continue
    }
    let best = calc.min(..ranks)
  ...
  for k in range(1, order.len() + 1) {
    ...
    curve.push((role: r.name, club: k, covered: running / songs.len()))
  }
  (name: r.name, curve: curve, uncredited: uncredited, club-size: order.len())
})
...
#let small-club = 10
#let large-club = 50
#let producers-small = covered-at("Producers", small-club)
#let writers-small = covered-at("Writers", small-club)
#assert(
  producers-small > writers-small + 0.1,
...
#let people = {
  let by-person = (:)
  for c in credits {
    let p = by-person.at(c.person, default: (person: c.person, wrote: 0, produced: 0))
    p.insert(if c.role == "Writers" { "wrote" } else { "produced" }, c.n)
    by-person.insert(c.person, p)
  }
  by-person
    .values()
...
#compose(
  // Left panel: how much of the chart the most-credited people cover, club size
  // by club size. The axis is logarithmic because the shape is in the first
  // dozen names, and a linear axis would spend its width on the tail.
  defer(
    plot,
    data: covered.map(c => c.curve).flatten(),
    ...
    layers: (
      ...
      annotate(
        ...
        label: chip(width: 2.65cm, text(font: body-font, size: 6.5pt, fill: note-colour)[
          At the top #small-club names, #lower(role-order.last()) cover #pct1(producers-small) of the chart and #lower(role-order.first()) #pct1(writers-small): #pct1(producers-small - writers-small) apart, on the same club size.
        ]),
        anchor: "west",
        clip: false,
      ),
    ...
    scales: scales(
      ...
      colour: scale-discrete(limits: role-order, palette: role-colours.values()),
    ),
  ...
  defer(
    ...
    layers: (
      ...
      // The names the panel calls out. Each one must still carry credits, so a
      // name that leaves the data fails the render. The offsets point at the
      // middle of the panel, in credits, so no label reaches for an edge and the
      // panel needs no room made for it.
      geom-text(
        data: (
          ("Dann Huff", 9, -7),
          ...
          let hit = people.find(p => p.person == person)
          assert(hit != none, message: "no credits left for " + person)
          (..hit, nudge-x: dx, nudge-y: dy)
        }),
        mapping: aes(label: "person", nudge-x: "nudge-x", nudge-y: "nudge-y"),
        size: 6pt,
        font: body-font,
        colour: note-colour,
        segment: true,
  ...
  labels: labels(
    ...
    subtitle: [
      ...
      Ranked by hits, the top #small-club producers sit on #pct1(producers-small) of the chart and the top #small-club writers on #pct1(writers-small)#[;] by #large-club names it is #pct1(covered-at("Producers", large-club)) against #pct1(covered-at("Writers", large-club)). \
