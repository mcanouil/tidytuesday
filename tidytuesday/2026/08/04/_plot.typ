...
#for r in raw {
  ...
  // one more, so weight comes from `net_wgt`, which is present on every row.
  let t = float(r.net_wgt) / 1000
...
#let years = tally.pairs().map(((key, acc)) => {
  ...
  // Where a year's kilos sit on the calendar: a vector mean over the twelve
  // months, weighted by weight. The mean holds only while the concentration `R`
  // stays high, and the caption reports how low it gets.
  let s = 0.0
  let c = 0.0
  for (i, w) in acc.months.enumerate() {
  ...
  let month = ang / (2 * calc.pi) * 12 + 1
  (
    year: int(key),
    label: key,
    // The clip is sheared in the southern spring, so a calendar year splits one
    // season. Months run September to August: September is 1, April is 8.
    // Nothing reaches past 9, so the axis never wraps.
    arrival: calc.rem(month - 9 + 12, 12) + 1,
...
#let binned = years.map(y => {
  let (dx, dy) = label-nudges.at(y.label)
  y + (
    bin: tonne-labels.at((2500, 5000, 7500).filter(e => y.tonnes >= e).len()),
    nudge-x: dx * 1cm,
    nudge-y: dy * 1cm,
  )
})
...
#plot(
  ...
  layers: (
    // The path: one hop per pair of consecutive years, bowed so the excursion
    // reads as a journey out and back. At 0.5 the return leg would cross the
    // outbound one. Each hop carries the year it leaves, so the thread can be
    // shaded.
    geom-curve(
      data: years.windows(2).map(((a, b)) => (
        arrival: a.arrival,
        share: a.share,
        xend: b.arrival,
        yend: b.share,
      ...
      mapping: aes(xend: "xend", yend: "yend", colour: "from"),
      curvature: 0.22,
      stroke: 1.1pt,
    ),
    // Weight twice over: circle area, and one of four fill bins. The smallest
    // circles are also the palest, and a pale fill on a small mark disappears.
    // The ring is what stops it: every circle keeps the same outline whatever its
    // bin. The ring is the accent colour, not white: white separates circles on
    // the pale page but cuts a bright gap in them on the dark one.
    geom-point(
  ...
  scales: scales(
    ...
    y: scale-continuous(
      ...
      labels: format-percent(scale: 1),
      expand: (18%, 10%),
    ),
    size: scale-area(range: (2.5pt, 9pt)),
    // The thread is shaded along its length, so the excursion reads as a
    // direction. The ramp gains saturation and holds its lightness: 2010 leaves
    // grey, 2024 arrives blue. A pale-to-dark ramp would fail against one page or
    // the other.
    colour: scale-gradient(low: rgb("#b6bac1"), high: rgb("#15618f")),
    fill: scale-discrete(
      limits: tonne-labels,
      // Four steps, all lighter than the dark theme's page. A ramp running down
      // into deep blue separates the bins on white but collapses the top two on
      // black. Keeping every step light costs contrast at the pale end, which the
      // ring covers.
      palette: (rgb("#dceaf4"), rgb("#a3c9e2"), rgb("#6aa4c9"), rgb("#2f7fc4")),
    ),
  ),
  ...
  labels: labels(
    ...
    subtitle: [
      ...
      Watch the circles shrink as it goes: #peak.label sent #format-percent()(peak.share / 100) east, but of #comma(calc.round(peak.tonnes)) tonnes against #comma(calc.round(heaviest.tonnes)) in #heaviest.label.
    ],
    ...
    caption: typst([
      #comma(calc.round(total-tonnes)) tonnes over #raw.len() monthly records, #comma(calc.round(first-year.tonnes)) to #comma(calc.round(heaviest.tonnes)) a year. A year's month is a weight-weighted vector mean; concentration runs #dp2(flattest.concentration) to #dp2(calc.max(..years.map(y => y.concentration))). \
