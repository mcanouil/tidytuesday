...
// dark toggle. A standalone compile sets no page fill, so white is the fallback.
...
#let panel(country, annotations) = defer(
  plot,
  data: csv("data/energy_cleaned.csv", row-type: dictionary)
  ...
  scales: scales(
    ...
    fill: scale-discrete(
      limits: cats,
      palette: cat-colours.values(),
      // One definition per category, in the order of `cats`. Each names the raw
      // columns summed above, so the legend says what every band holds.
      labels: (
...
#box[
  #compose(
    // In-panel callouts: the total renewable share at each end, and a one-line
    // reason in the empty corner. The text takes the theme ink, so it follows
    // the toggle.
    panel("Vietnam", (
